...
class Floor1 extends Floor implements Cloneable {
	...
	protected void computeFloor(final float[] vector) {
		...
		for (int i = 2; i < values; i++) {
			...
			if (val != 0) {
				...
				step2Flags[i] = true;
				if (val >= room) {
					yList[i] = highRoom > lowRoom ? val - lowRoom + predicted : -val + highRoom
							+ predicted - 1;
				} else {
					yList[i] = (val & 1) == 1 ? predicted - ((val + 1) >> 1) : predicted
							+ (val >> 1);
				}
			} else {
				step2Flags[i] = false;
	...
	protected Floor decodeFloor(VorbisStream vorbis, BitInputStream source)
		...
		for (int i = 0; i < partitionClassList.length; i++) {
			...
			int cval = 0;
			if (cbits > 0) {
				cval = source.getInt(vorbis.getSetupHeader().getCodeBooks()[classMasterbooks[cls]]
						.getHuffmanRoot());
			...
			for (int j = 0; j < cdim; j++) {
				...
				cval >>>= cbits;
				if (book >= 0) {
					clone.yList[j + offset] = source
							.getInt(vorbis.getSetupHeader().getCodeBooks()[book].getHuffmanRoot());
