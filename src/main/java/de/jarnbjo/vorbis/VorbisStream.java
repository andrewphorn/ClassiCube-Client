...
public class VorbisStream {
    ...
    public byte[] processPacket(byte[] packet) throws VorbisFormatException, IOException {
        ...
        if (((int) packet[0] & 1) == 1) {
            ...
            // audio packet
            if (identificationHeader == null || commentHeader == null || setupHeader == null) {

                throw new VorbisFormatException(
                        "Cannot decode audio packet before all three header packets have been decoded.");
            }
        ...
        synchronized (streamLock) {
            ...
            if (currentPcm == null || currentPcmIndex >= currentPcmLimit) {
                ...
                try {
                    ...
                    currentPcmLimit = ap.getNumberOfSamples() * identificationHeader.getChannels()
                            * 2;
                } catch (ArrayIndexOutOfBoundsException e) {
                    return 0;
                }
