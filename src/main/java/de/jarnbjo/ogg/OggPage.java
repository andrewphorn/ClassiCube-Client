...
public class OggPage {
    ...
    private static OggPage create(Object source, boolean skipData) throws IOException,
        ...
        try {
            ...
            if (capture != 0x5367674f) {
                ...
                cs = cs.substring(6, 8) + cs.substring(4, 6) + cs.substring(2, 4)
                        + cs.substring(0, 2);
                ...
                System.out.println("Ogg packet header is 0x" + cs + " (" + c1 + c2 + c3 + c4
                        + "), should be 0x4f676753 (OggS)");
            }
            ...
            return new OggPage(version, bf1, bos, eos, absoluteGranulePosition, streamSerialNumber,
                    pageSequenceNumber, pageCheckSum, segmentOffsets, segmentLengths, totalLength,
    ...
    private OggPage(int version, boolean continued, boolean bos, boolean eos,
            long absoluteGranulePosition, int streamSerialNumber, int pageSequenceNumber,
            int pageCheckSum, int[] segmentOffsets, int[] segmentLengths, int totalLength,
            byte[] header, byte[] segmentTable, byte[] data) {
