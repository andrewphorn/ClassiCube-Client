...
public class CachedUrlStream implements PhysicalOggStream {
	...
	public void run() {
		...
		while (!eos) {
			...
			long pos = listSize > 0 ? ((Long) pageOffsets
				.get(listSize - 1)).longValue()
			...
			if (drain != null) {
			    ...
			    System.arraycopy(arr1, 0, memoryCache, (int) pos,
				    arr1.length);
			    System.arraycopy(arr2, 0, memoryCache, (int) pos
				    + arr1.length, arr2.length);
			    System.arraycopy(arr3, 0, memoryCache, (int) pos
				    + arr1.length + arr2.length, arr3.length);
			}

			pageOffsets.add(new Long(pos));
			pageLengths.add(new Long(arr1.length + arr2.length
				+ arr3.length));
		    }
			...
			logicalStreams.put(
				new Integer(op.getStreamSerialNumber()), los);
			los.checkFormat(op);
		    }
