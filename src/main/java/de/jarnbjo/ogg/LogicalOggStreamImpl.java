...
public class LogicalOggStreamImpl implements LogicalOggStream {
    ...
    public void checkFormat(OggPage page) {
        ...
        if (data.length >= 7 && data[1] == 0x76 && data[2] == 0x6f && data[3] == 0x72
                && data[4] == 0x62 && data[5] == 0x69 && data[6] == 0x73) {

            format = FORMAT_VORBIS;
        } else if (data.length >= 7 && data[1] == 0x74 && data[2] == 0x68 && data[3] == 0x65
                && data[4] == 0x6f && data[5] == 0x72 && data[6] == 0x61) {

            format = FORMAT_THEORA;
        } else if (data.length == 4 && data[0] == 0x66 && data[1] == 0x4c && data[2] == 0x61
                && data[3] == 0x43) {
    ...
    public synchronized byte[] getNextOggPacket() throws
        ...
        do {
            ...
            res.write(currentPage.getData(), currentPage.getSegmentOffsets()[currentSegmentIndex],
                    segmentLength);
            currentSegmentIndex++;
