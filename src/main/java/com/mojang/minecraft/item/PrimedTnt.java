...
public class PrimedTnt extends Entity {
	...
	if (!defused) {
		...
		for (int i = 0; i < 100; i++) {
		    float unknown0 = (float) random.nextGaussian() * radius
			    / 4.0F;
		    float unknown1 = (float) random.nextGaussian() * radius
			    / 4.0F;
		    float unknown2 = (float) random.nextGaussian() * radius
			    / 4.0F;
		    ...
		    TerrainParticle terrainParticle = new TerrainParticle(
			    level, x + unknown0, y + unknown1, z + unknown2,
			    unknown4, unknown5, unknown3, Block.TNT);
