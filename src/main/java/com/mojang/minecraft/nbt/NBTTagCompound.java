...
public class NBTTagCompound extends NBTBase {
	...
	public double getDouble(String name) {
		try {
			return !this.tagMap.containsKey(name) ? 0D
					: ((NBTTagDouble) this.tagMap.get(name)).data;
		} catch (ClassCastException e) {
			throw e;
		}
	}
	...
	public String getString(String name) {
		try {
			return !this.tagMap.containsKey(name) ? ""
					: ((NBTTagString) this.tagMap.get(name)).data;
		} catch (ClassCastException e) {
			throw e;
		}
	}
	...
	public byte[] getByteArray(String name) {
		try {
			return !this.tagMap.containsKey(name) ? new byte[0] : ((NBTTagByteArray) this.tagMap
					.get(name)).byteArray;
		} catch (ClassCastException e) {
			throw e;
		}
	...
	public int[] getIntArray(String name) {
		try {
			return !this.tagMap.containsKey(name) ? new int[0] : ((NBTTagIntArray) this.tagMap
					.get(name)).intArray;
		} catch (ClassCastException e) {
			throw e;
		}
	...
	public NBTTagCompound getCompoundTag(String name) {
		try {
			return !this.tagMap.containsKey(name) ? new NBTTagCompound(name)
	...
	public NBTTagList getTagList(String name) {
		try {
			return !this.tagMap.containsKey(name) ? new NBTTagList(name) : (NBTTagList) this.tagMap
					.get(name);
		} catch (ClassCastException e) {
			throw e;
		}
	...
	public String toString() {
		...
		for (Iterator<String> iter = this.tagMap.keySet().iterator(); iter.hasNext(); s = s + s1
				+ ":" + this.tagMap.get(s1) + ",") {
			s1 = iter.next();
		}
