...
public class BlockMap implements Serializable {
    ...
    public List<Entity> getEntities(Entity entity, float x1, float y1, float z1, float x2, float y2, float z2, List<Entity> entityListToChange) {
        ...
        for (int i = BlockMap$Slot.getXSlot(thisSlot) - 1; i <= BlockMap$Slot.getXSlot(otherSlot) + 1; ++i) {
            for (int j = BlockMap$Slot.getYSlot(thisSlot) - 1; j <= BlockMap$Slot.getYSlot(otherSlot) + 1; ++j) {
                for (int k = BlockMap$Slot.getZSlot(thisSlot) - 1; k <= BlockMap$Slot.getZSlot(otherSlot) + 1; ++k) {
                    if (i >= 0 && j >= 0 && k >= 0 && i < width && j < depth && k < height) {
                        ...
                        for (int l = 0; l < entitySlotInGrid.size(); ++l) {
                            Entity theEntity;
                            if ((theEntity = (Entity) entitySlotInGrid.get(l)) != entity && theEntity.intersects(x1, y1, z1, x2, y2, z2)) {
        ...
        for (int var5 = 0; var5 < width; ++var5) {
            ...
            for (int var8 = 0; var8 < depth; ++var8) {
                ...
                for (int var11 = 0; var11 < height; ++var11) {
                    ...
                    if ((entitySlotInGrid = entityGrid[(var11 * depth + var8) * width + var5]).size() != 0) {
                        ...
                        if (frustrum.isBoxInFrustum(var6, var9, var13, var7, var10, var14)) {
                            ...
                            for (int var23 = 0; var23 < entitySlotInGrid.size(); ++var23) {
                                Entity var22;
                                if ((var22 = (Entity) entitySlotInGrid.get(var23)).shouldRender(var1)) {
                                    if (!var21) {
                                        AABB var24 = var22.bb;
