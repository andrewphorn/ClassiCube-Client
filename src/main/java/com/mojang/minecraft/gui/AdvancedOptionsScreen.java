...
public final class AdvancedOptionsScreen extends GuiScreen {
	...
	for (int var1 = 10; var1 < this.settings.settingCount; ++var1) {
	    this.buttons.add(new OptionButton(var1, this.width / 2 - 155
		    + heightSeperator % 2 * 160, this.height / 6 + 24
		    * (heightSeperator >> 1), this.settings.getSetting(var1)));
	    heightSeperator++;
	}
	this.buttons.add(new OptionButton(100, this.width / 2 - 155
		+ heightSeperator % 2 * 160, this.height / 6 + 24
		* (heightSeperator >> 1), "Water Level"));
	heightSeperator++;
	this.buttons.add(new OptionButton(200, this.width / 2 - 155
		+ heightSeperator % 2 * 160, this.height / 6 + 24
		* (heightSeperator >> 1), "Sky Color"));
	heightSeperator++;
	this.buttons.add(new OptionButton(300, this.width / 2 - 155
		+ heightSeperator % 2 * 160, this.height / 6 + 24
		* (heightSeperator >> 1), "Cloud Color"));
	heightSeperator++;
	this.buttons.add(new OptionButton(400, this.width / 2 - 155
		+ heightSeperator % 2 * 160, this.height / 6 + 24
		* (heightSeperator >> 1), "Fog Color"));
	heightSeperator++;
	this.buttons.add(new OptionButton(500, this.width / 2 - 155
		+ heightSeperator % 2 * 160, this.height / 6 + 24
		* (heightSeperator >> 1), "Sunlight Color"));
	heightSeperator++;
	this.buttons.add(new OptionButton(600, this.width / 2 - 155
		+ heightSeperator % 2 * 160, this.height / 6 + 24
		* (heightSeperator >> 1), "Shadow Color"));
	...
	if (var1.active) {
	    ...
	    if (var1.id == 100) {
		WaterLevelInputScreen screen = new WaterLevelInputScreen(
			parent, "" + this.minecraft.level.waterLevel, height,
		...
		SkyColorInputScreen screen = new SkyColorInputScreen(parent, ""
			+ Integer.toHexString(this.minecraft.level.skyColor),
			height, "Enter new value for sky color...");
		screen.allowedChars = "ABCDEFabcdef1234567890";
		screen.stringLimit = 6;
		this.minecraft.setCurrentScreen(screen);
	    }
	    if (var1.id == 300) {
		CloudColorInputScreen screen = new CloudColorInputScreen(
			parent,
			""
				+ Integer
					.toHexString(this.minecraft.level.cloudColor),
			height, "Enter new value for cloud color...");
		screen.allowedChars = "ABCDEFabcdef1234567890";
		screen.stringLimit = 6;
		this.minecraft.setCurrentScreen(screen);
	    }
		...
		FogColorInputScreen screen = new FogColorInputScreen(parent, ""
			+ Integer.toHexString(this.minecraft.level.fogColor),
			height, "Enter new value for fog color...");
		screen.allowedChars = "ABCDEFabcdef1234567890";
		screen.stringLimit = 6;
		this.minecraft.setCurrentScreen(screen);
	    }
		...
		String colorString = "";
		if (c != null)
		    colorString = String.format("%02x%02x%02x", (int)(c.R * 255), (int)(c.G * 255), (int)(c.B * 255));
		else
		    colorString = String.format("%02x%02x%02x",
			    color.getRed(), color.getGreen(), color.getBlue());
		LightColorInputScreen screen = new LightColorInputScreen(
			parent, "" + colorString, height,
			"Enter new value for light color...");
		screen.allowedChars = "ABCDEFabcdef1234567890";
		screen.stringLimit = 6;
		this.minecraft.setCurrentScreen(screen);
	    }
		...
		if (c != null)
		    ...
		    colorString = String.format("%02x%02x%02x",
			    color.getRed(), color.getGreen(), color.getBlue());
		ShadowColorInputScreen screen = new ShadowColorInputScreen(
			parent, "" + colorString, height,
			"Enter new value for shadow color...");
		screen.allowedChars = "ABCDEFabcdef1234567890";
		screen.stringLimit = 6;
		this.minecraft.setCurrentScreen(screen);
	    }
