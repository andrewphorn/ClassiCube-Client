...
public final class SoundReader {
	...
	try {
	    LogicalOggStreamImpl OggStream = (LogicalOggStreamImpl) (new OnDemandUrlStream(
		    var0)).getLogicalStreams().iterator().next();
	...
	label51: while (var3 >= 0) {
		...
		try {
		    if (var15 < var2.length
			    && (var3 = vorbisStream.readPcm(var2, var15,
				    var2.length - var15)) > 0) {
			var15 += var3;
			continue;
		    }
	...
	return new SoundData(var5,
		(float) (var13 = vorbisStream.getIdentificationHeader())
			.getSampleRate());
    }
}
