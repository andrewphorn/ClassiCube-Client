...
public final class Music implements Audio {
	...
	this.player = var1;
	try {
	    LogicalOggStreamImpl var3 = (LogicalOggStreamImpl) (new OnDemandUrlStream(
		    var2)).getLogicalStreams().iterator().next();
	...
	if (!this.player.settings.music) {
	    ...
	    while (var3 > 0
		    && (this.processing != null || this.previous != null)) {
	    ...
	    return this.processing != null || this.previous != null
		    || !this.finished;
	}
    }
}
